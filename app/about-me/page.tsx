...
function pluralize(num: number, one: string, few: string, many: string) {
  const mod10 = num % 10;
...
function calculateExperience(startDate: string) {
  ...
  let months = now.getMonth() - start.getMonth() + 4;
  ...
  const monthStr = `${months} ${pluralize(months, 'месяц', 'месяца', 'месяцев')}`;

  if (years > 0 && months > 0) return `${yearStr} и ${monthStr}`;
  ...
  const menuItems = data.aboutMe.menuItems.map((item) =>
    React.cloneElement(iconComponents[item.icon], {
      ...
      className: `cursor-pointer hover:opacity-100 ${
        selectedMenu === item.key ? 'opacity-100' : 'opacity-40'
      }`,
    })
  );
      ...
      {(state) => (
        ...
        >
          <div className="flex flex-col xl:flex-row">
            ...
            <Menu items={menuItems} onSelect={setSelectedMenu} />
            <SubMenu
          ...
          <div className="flex flex-col h-content xl:grid xl:grid-cols-2 w-full xl:h-full">
            <Content
              ...
              content={
                selectedMenu === 0 && selectedSubMenu === 0
                  ? `Опыт разработки - ${experienceFormatted}.` +
                    contents[selectedMenu][selectedSubMenu]
                  : contents[selectedMenu][selectedSubMenu]
              }
            />
            <Snippets />
